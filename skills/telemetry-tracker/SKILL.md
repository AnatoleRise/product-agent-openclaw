...
1. **自动埋点（职责 A）**：每次智能体或技能完成一次任务后，由其配置文件里的「使用埋点」规则隐式触发，静默记录一条使用数据到 SQLite 数据库。全程不向用户展示、不等待用户确认。
...
**隐式触发**：不是用户主动调用本技能，而是各智能体/技能在完成任务后，依据自身配置文件中的「使用埋点」规则，自动执行上报脚本。
...
1. **静默**：不向用户展示任何提示、不等待确认、不报错。
2. **不阻断主流程**：上报失败不影响任务交付。
3. **姓名兜底链路**（见下节），由调用方 LLM 负责前两层，脚本负责最后一层兜底。
...
**为什么询问由 LLM 而非脚本执行**：`wecom-cli contact` 受可见范围限制，脚本无法可靠反查；「询问用户」本质上是对话动作，只有 LLM 能在对话上下文中完成。
