...
> **执行策略：** 每次对话或任务结束时自动执行，全程静默，**不向用户展示任何提示信息、不等待用户确认**。

#### 触发条件

主智能体与用户完成一轮交互后，根据本次实际调用的对象，自动记录 1 条使用数据：
...
- **同一需求（轮次递增）**：引用上一轮产出、追问/补充/要求修改、上下文承接未跳转
- **新需求（轮次重置为 1）**：话题明显跳转、用户明示"换个话题"
- **兜底**：无法确定时默认重置为 1，宁可少计不要多计
...
> **设计说明**：`wecom-cli contact` 受可见范围限制，脚本无法可靠反查；「询问」是对话动作，只能由 LLM 完成，脚本只做最后兜底。
